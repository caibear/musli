    ...
    L: UsizeEncoding,
{
    ...
    fn encode_bytes(mut self, bytes: &[u8]) -> Result<(), Self::Error> {
        let (tag, embedded) = TypeTag::with_len(TypeKind::Prefixed, bytes.len());
    ...
    fn encode_bytes_vectored(mut self, vectors: &[&[u8]]) -> Result<(), Self::Error> {
        ...
        let (tag, embedded) = TypeTag::with_len(TypeKind::Prefixed, len);
        self.writer.write_byte(tag.byte())?;
    ...
    fn encode_str(mut self, string: &str) -> Result<(), Self::Error> {
        let (tag, embedded) = TypeTag::with_len(TypeKind::Prefixed, string.len());
        self.writer.write_byte(tag.byte())?;
    ...
    fn encode_bool(self, value: bool) -> Result<(), Self::Error> {
        self.writer
            .write_byte(TypeTag::new(TypeKind::Fixed, if value { 1 } else { 0 }).byte())
    }
    ...
    fn encode_u8(self, value: u8) -> Result<(), Self::Error> {
        let (tag, embedded) = TypeTag::with_byte(TypeKind::Byte, value);
        self.writer.write_byte(tag.byte())?;
    ...
    fn encode_some(self) -> Result<Self::Some, Self::Error> {
        self.writer
            .write_byte(TypeTag::new(TypeKind::Mark, 1).byte())?;
        Ok(self)
    }
    ...
    fn encode_none(self) -> Result<(), Self::Error> {
        self.writer
            .write_byte(TypeTag::new(TypeKind::Mark, 0).byte())?;
        Ok(())
    }

    #[inline]
    fn encode_sequence(self, len: usize) -> Result<Self::Sequence, Self::Error> {
        let (tag, embedded) = TypeTag::with_len(TypeKind::Sequence, len);
        self.writer.write_byte(tag.byte())?;
    ...
    fn encode_map(self, len: usize) -> Result<Self::Map, Self::Error> {
        let (tag, embedded) = TypeTag::with_len(TypeKind::PairSequence, len);
        self.writer.write_byte(tag.byte())?;
    ...
    fn encode_struct(self, fields: usize) -> Result<Self::Struct, Self::Error> {
        let (tag, embedded) = TypeTag::with_len(TypeKind::PairSequence, fields);
        self.writer.write_byte(tag.byte())?;
    ...
    fn encode_tuple(self, len: usize) -> Result<Self::Tuple, Self::Error> {
        let (tag, embedded) = TypeTag::with_len(TypeKind::Continuation, len);
        self.writer.write_byte(tag.byte())?;
    ...
    fn encode_variant(self) -> Result<Self::Variant, Self::Error> {
        self.writer
            .write_byte(TypeTag::new(TypeKind::PairSequence, 1).byte())?;
        Ok(self)
    }
}
